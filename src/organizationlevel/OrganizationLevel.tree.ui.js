...
import { handleRequestSort, setPage, loadEntity, createEntity, loadChildren, close, deleteEntities, handleReadValidation } from './organizationLevel.actions'
import ConfirmDialog from '../common/Confirm.dialog';

function EnhanceAlert() {
  const dispatch = useDispatch()
  const validations = useSelector(state => state.organizationLevel.validations.filter(item => !item.read)) || [];
...
function EnhancedTableHead(props) {
  ...
  const messages = useSelector(state => state.organizationLevel.messages) || {};
...
const EnhancedTableToolbar = (props) => {
  ...
  const handleCreate = () => {
    ...
    history.push('/organizationLevel/form')
  }
...
const EnhancedTableTRow = (props) => {
  ...
  const lastUpdate = useSelector(state => state.organizationLevel.lastUpdate);
...
export default function EnhancedTable() {
  ...
  const order = useSelector(state => state.organizationLevel.table.order);
  const orderBy = useSelector(state => state.organizationLevel.table.orderBy);
  const list = useSelector(state => state.organizationLevel.table.list);
  const page = useSelector(state => state.organizationLevel.table.pageItem.page);
  const total = useSelector(state => state.organizationLevel.table.pageItem.total);
  const rowsPerPage = useSelector(state => state.organizationLevel.table.pageItem.rowsPerPage);
  const messages = useSelector(state => state.organizationLevel.messages) || {};
  const baseMessages = useSelector(state => state.base.messages) || {};
  const history = useHistory();
