...
import { handleRequestSort, setPage, loadEntity, createEntity, loadChildren, close, deleteEntities, handleReadValidation, loadOrganizationLevel } from './organizationUnit.actions'
...
const EnhancedTableToolbar = (props) => {
  ...
  const handleCreate = () => {
    dispatch(createEntity(null, modelId))
    dispatch(loadOrganizationLevel(null))
    history.push('/organizationUnit/form')
  }
...
const EnhancedTableTRow = (props) => {
  ...
  const lastUpdate = useSelector(state => state.organizationUnit.lastUpdate);
  ...
  const handleCreate = (id, e) => {
    dispatch(createEntity(id, modelId));
    handleClose(e);
    history.push('/organizationUnit/form')
  }
      ...
      >
        ...
        <TableCell>
          <IconButton aria-label="expand row" size="small" onClick={(event) => handleClick(event)}>
            ...
            >
              <MenuItem onClick={e => handleCreate(props.row.id, e)} >
...
export default function EnhancedTable() {
  ...
  const order = useSelector(state => state.organizationUnit.table.order);
  const orderBy = useSelector(state => state.organizationUnit.table.orderBy);
  const list = useSelector(state => state.organizationUnit.table.list);
  const page = useSelector(state => state.organizationUnit.table.pageItem.page);
  const total = useSelector(state => state.organizationUnit.table.pageItem.total);
  const rowsPerPage = useSelector(state => state.organizationUnit.table.pageItem.rowsPerPage);
  const messages = useSelector(state => state.organizationUnit.messages) || {};
  const baseMessages = useSelector(state => state.base.messages) || {};
  const history = useHistory();
