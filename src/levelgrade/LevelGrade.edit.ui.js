...
import { saveOrUpdate, inputChangeHandler, requiredValidation } from './levelgrade.actions'
...
function EnhanceAlert() {
    ...
    const validations = useSelector(state => state.levelGrade.validations.filter(item => !item.read)) || [];
...
export default function ScrollDialog(props) {
    ...
    const dispatch = useDispatch();
    const messages = useSelector(state => state.levelGrade.messages) || {}
    const baseMessages = useSelector(state => state.base.messages) || {};
    const validationsLen = useSelector(state => state.levelGrade.validations.length) || 0;
    const validations = useSelector(state => state.levelGrade.validations) || [];
    const name = useSelector(state => state.levelGrade.entity.name);
    const code = useSelector(state => state.levelGrade.entity.code);
    const levelId = useSelector(state => state.levelGrade.entity.levelId) || "";
    const status = useSelector(state => state.levelGrade.status);
    const organizationLevels = useSelector(state => state.organizationLevel.table.list) || [];
    const history = useHistory();

    if (status == 'created') {
        history.push("/levelGrade/list")
    }
        ...
        <div>
            ...
            <Paper className={classes.paper} elevation={0}>
                ...
                <FormControl fullWidth className={classes.formControl}
                    variant="outlined"
                    required >
                    <InputLabel id="levelGrade-organizationLevel-label">{messages.organizationLevel}</InputLabel>
                    <Select
                        label={messages.organizationLevel}
                        labelId="levelGrade-organizationLevel-label"
                        id="levelGrade-organizationLevel-outlined"
                        value={levelId}
                        onChange={e => onTextFieldChange(e, 'levelId')}>
                        <MenuItem value="">
                            <em>{"-"}</em>
                        </MenuItem>
                        {organizationLevels.map((item, index) => {
                            return <MenuItem key={index} value={item.id}>{item.name}</MenuItem>
                        })}

                    </Select>
                    {
                        displayValidationText('organizationLevel').length > 0 ? <FormHelperText>{displayValidationText('organizationLevel')}</FormHelperText>
                            : <></>
                    }
                </FormControl>
                ...
                <Button component={(props) => <Link to={"/levelGrade/list"} {...props} />}
                    color="default"
                    variant="contained"
