...
import { saveOrUpdate, inputChangeHandler, requiredValidation } from './activity.actions'
...
const activityTypes = () => {
    ...
    return [
        { value: 'STRATEGICGROUP', label: messages.STRATEGICGROUP },
        { value: 'PROCESSGROUP', label: messages.PROCESSGROUP },
        { value: 'ACTIVITYUNIT', label: messages.ACTIVITYUNIT },
...
export default function ScrollDialog(props) {
    ...
    const validationsLen = useSelector(state => state.activity.validations.length) || 0;
    const validations = useSelector(state => state.activity.validations) || [];
    const name = useSelector(state => state.activity.entity.name);
    const code = useSelector(state => state.activity.entity.code);
    const type = useSelector(state => state.activity.entity.type);
    const driverId = useSelector(state => state.activity.entity.driverId);
    const description = useSelector(state => state.activity.entity.description);
    const status = useSelector(state => state.activity.status);

    const drivers = useSelector(state => state.drivers.table.list).filter(item=> item.type == 'ACTIVITY') || [];
        ...
        <div>
            ...
            <Paper className={classes.paper} elevation={0}>
                ...
                <FormControl fullWidth className={classes.formControl}
                    ...
                    required >
                    <InputLabel id="activity-type-label">{messages.type}</InputLabel>
                    <Select
                        label={messages.type}
                        labelId="activity-type-label"
                        id="activity-type-outlined"
                        value={type}
                ...
                <FormControl fullWidth className={classes.formControl}
                    variant="outlined"
                     >
                    <InputLabel id="activity-driver-label">{messages.driver}</InputLabel>
                    <Select
                        label={messages.driver}
                        labelId="activity-driver-label"
                        id="activity-driver-outlined"
                        value={driverId}
                ...
                <TextField
                    ...
                    onBlur={e => validateHandler('description')}
                ...
                <Button component={(props) => <Link to={"/activity/list"} {...props} />}
                    color="default"
                    variant="contained"
