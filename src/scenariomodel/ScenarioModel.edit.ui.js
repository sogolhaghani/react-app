...
import { saveOrUpdate, inputChangeHandler, requiredValidation } from './scenariomodel.actions'
...
export default function ScrollDialog(props) {
    ...
    const validationsLen = useSelector(state => state.scenarioModel.validations.length) || 0;
    const validations = useSelector(state => state.scenarioModel.validations) || [];
    const name = useSelector(state => state.scenarioModel.entity.name);
    const code = useSelector(state => state.scenarioModel.entity.code);
...
<Typography variant="h5" component="h4">
            ...
            <Paper className={classes.paper} elevation={0}>
                    <TextField
                        ...
                        error={dispayValidationText('name').length > 0}
                        variant="outlined"
                        margin="normal"
                        required
                        fullWidth
                        id="name"
                        label={messages.name}
                        helperText={dispayValidationText('name')}
                        value={name} />
                    <TextField
                        FormHelperTextProps={{
                            className: classes.helperText
                        ...
                        error={dispayValidationText('code').length > 0}
                        variant="outlined"
                        margin="normal"
                        required
                        fullWidth
                        id="code"
                        helperText={dispayValidationText('code')}
...
<Button onClick={submitHandler} color="primary" variant="contained" className={classes.button}>
                ...
                <Button component={(props) => <Link to={"/model/list"} {...props} />}
                    color="default"
                    variant="contained"
