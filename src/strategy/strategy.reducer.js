...
import * as type from './strategy.actions';
import messages from './strategy.messages';
...
const initState = {
  ...
  entity: { hasIncome: false },
  validations: [],
  status: 'init'
}

const strategyType = ['GOAL', 'OBJECTIVE', 'STRATEGY', 'TARGET', 'ACTIONPLAN']
...
export default function reducer(state = initState, action) {
  ...
  switch (action.type) {
    ...
    case type.CREATE_ENTITY:
      ...
      newState.entity = { hasIncome: false }
      newState.validations = []
      if (action.parentId) {
        newState.entity.parentId = action.parentId
        newState.entity.isRoot = false
        newState.entity.type = strategyType[strategyType.indexOf(action.strategyType) + 1]
      } else {
        newState.entity.isRoot = true
        newState.entity.type = strategyType[0]
      }
