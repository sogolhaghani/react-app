...
import { handleRequestSort, setPage, loadEntity, createEntity, deleteEntities, handleReadValidation } from './drivers.actions'
...
function EnhancedTableHead(props) {
  ...
  const headCells = useSelector(state => state.drivers.headCells) || []
...
export default function EnhancedTable() {
  ...
  const order = useSelector(state => state.drivers.table.order);
  const orderBy = useSelector(state => state.drivers.table.orderBy);
  const list = useSelector(state => state.drivers.table.list);
  const page = useSelector(state => state.drivers.table.pageItem.page);
  const total = useSelector(state => state.drivers.table.pageItem.total);
  const rowsPerPage = useSelector(state => state.drivers.table.pageItem.rowsPerPage);
                ...
                .map((row, index) => {
                  ...
                  return (
                   ...
                   >
                      ...
                      <TableCell >{messages[row.type]}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
