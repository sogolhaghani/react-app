...
import { handleRequestSort, setPage, loadEntity, createEntity, loadChildren, close, deleteEntities, handleReadValidation } from './strategy.actions'
    ...
    <TableHead>
      <TableRow>
        ...
        <TableCell
          ...
          align={'left'}
          padding={'default'}
          sortDirection={orderBy === "driver" ? order : false}
        >
          <TableSortLabel
...
const EnhancedTableTRow = (props) => {
  ...
  const lastUpdate = useSelector(state => state.strategy.lastUpdate);
  ...
  return (
    <>
      ...
      >
        ...
        <TableCell style={{ color: lighten('#18202c', props.row.level * 0.1) }} align="left">{props.row.driverName}</TableCell>
        <TableCell style={{ color: lighten('#18202c', props.row.level * 0.1) }} align="left">{messages[props.row.type]}</TableCell>
        <TableCell>
...
export default function EnhancedTable() {
  ...
  const order = useSelector(state => state.strategy.table.order);
  const orderBy = useSelector(state => state.strategy.table.orderBy);
  const list = useSelector(state => state.strategy.table.list);
  const page = useSelector(state => state.strategy.table.pageItem.page);
  const total = useSelector(state => state.strategy.table.pageItem.total);
  const rowsPerPage = useSelector(state => state.strategy.table.pageItem.rowsPerPage);
  const messages = useSelector(state => state.strategy.messages) || {};
  const baseMessages = useSelector(state => state.base.messages) || {};
  const history = useHistory();
