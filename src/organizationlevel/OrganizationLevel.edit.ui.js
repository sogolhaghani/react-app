...
import { saveOrUpdate, inputChangeHandler, requiredValidation } from './organizationLevel.actions'
...
export default function ScrollDialog(props) {
    ...
    const validationsLen = useSelector(state => state.organizationLevel.validations.length) || 0;
    const validations = useSelector(state => state.organizationLevel.validations) || [];
    const name = useSelector(state => state.organizationLevel.entity.name);
    const code = useSelector(state => state.organizationLevel.entity.code);
    const type = useSelector(state => state.organizationLevel.entity.type);
    const status = useSelector(state => state.organizationLevel.status);

    const history = useHistory();
    if (status == 'created') {
        history.push("/organizationLevel/list")
    }
    ...
    return (
        <div>
            ...
            <Paper className={classes.paper} elevation={0}>
                ...
                <Button component={(props) => <Link to={"/organizationLevel/list"} {...props} />}
                    color="default"
                    variant="contained"
